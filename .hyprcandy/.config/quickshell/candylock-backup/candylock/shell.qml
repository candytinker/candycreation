...
ShellRoot {
    ...
    property string mediaShuffleStatus: "off"  // off, on
    property string mediaLoopStatus: "none"    // none, track, playlist
        ...
        stdout: SplitParser {
            ...
            onRead: function(l){
                ...
                if(p.length>=6){
                    ...
                    root.mediaShuffleStatus = p[4].trim() || "off"
                    root.mediaLoopStatus = p[5].trim() || "none"
                }
            }
        }
        Component.onCompleted: running=true
                            ...
                            Rectangle {
                                ...
                                ColumnLayout {
                                    ...
                                    Item {
                                        ...
                                        // Radial cava canvas — 64 bars around 96px radius circle
                                        Canvas {
                                            id:radialCava
                                            anchors.fill:parent
                                            ...
                                            property real _maxAngle: Math.PI * 2
                                            ...
                                            onPaint: {
                                                ...
                                                const innerR = 50, outerR = 71  // Increased bar height by 50% (14px → 21px)
                                                
                                                for (let i = 0; i < _barCount; i++) {
                                                    const angle = (i / _barCount) * _maxAngle - Math.PI / 2
                                                    const barH = _bars[i] * (outerR - innerR)
                                                    
                                                    ctx.beginPath()
                                                    ctx.strokeStyle = Qt.rgba(root.cPrimary.r, root.cPrimary.g, root.cPrimary.b, 0.7)
                                                    ctx.lineWidth = 3
                                                    ctx.lineCap = "round"
                                    ...
                                    RowLayout {
                                        ...
                                        Repeater {
                                            model:[
                                                {i:root.mediaShuffleStatus==="on"?"󰁠":"󰁡",c:"shuffle",a:root.mediaShuffleStatus==="on"},
                                                {i:"󰒮",c:"previous",a:false},
                                                {i:root.mediaStatus==="Playing"?"󰏤":"󰐊",c:"play-pause",a:false},
                                                {i:"󰒭",c:"next",a:false},
                                                {i:root.mediaLoopStatus==="track"?"󰑖":(root.mediaLoopStatus==="playlist"?"󰑗":"󰑕"),c:"loop",a:root.mediaLoopStatus!=="none"}
                                            ]
                                            delegate: Rectangle {
